...
6. **Sequence matters.** Roughly: `who-is-this-for` → `talk-to-users` → `positioning-and-story` → `value-prop-that-converts` → `the-homepage` → `first-50-users` / `launch-it` → `market-to-devs-sell-to-buyers` → `founder-led-content` → `know-if-its-working`. Don't optimize a homepage before you know who it's for.
